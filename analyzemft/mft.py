...
import analyzemft.bitparse as bitparse
import analyzemft.mftutils as mftutils
...
def mft_to_csv(record, ret_header, options):
    ...
    if record['fncnt'] > 0 and 'si' in record:
        ...
        filename_buffer = [
            ...
            options.date_formatter(record['si']['atime'].dtstr),
            options.date_formatter(record['si']['ctime'].dtstr),
            NO_FN_RECORD, NO_FN_RECORD, NO_FN_RECORD, NO_FN_RECORD,
        ]

    else:
        filename_buffer = [
            NO_FN_RECORD,
            NO_SI_RECORD, NO_SI_RECORD, NO_SI_RECORD, NO_SI_RECORD,
            NO_FN_RECORD, NO_FN_RECORD, NO_FN_RECORD, NO_FN_RECORD,
        ]
...
def mft_to_body(record, full, std):
    ...
    if record['fncnt'] > 0:
        ...
        if 'si' in record:
            ...
            rec_bodyfile = ("%s|%s|%s|%s|%s|%s|%s|%d|%d|%d|%d\n" %
                            ('0', CORRUPT_RECORD, '0', '0', '0', '0', '0', 0, 0, 0, 0))
...
def mft_to_l2t(record):
    ...
    if record['fncnt'] > 0:
        ...
        csv_string = ("%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s\n" % (
            ...
            'Corrupt Record', 'desc',
            'version', NO_FN_RECORD, record['seq'], '-', 'format', 'extra'))
