...
class MftSession:
    ...
    def process_mft_file(self):
        ...
        if self.options.output is not None:
            line = mft.mft_to_csv(None, True, self.options)
            self.file_csv.writerow(line)
        ...
        while raw_record:
            ...
            if record['ads'] > 0:
                for i in range(0, record['ads']):
                    ...
                    record_ads['filename'] = record['filename'] + b':' + record['data_name', i]
    ...
    def get_folder_path(self, seqnum):
        ...
        # Self referential parent sequence number. The filename becomes a NO_FN_RECORD note
        ...
        for i in self.mft:
            ...
            if (self.mft[i]['filename']) == '':
                ...
                if self.mft[i]['fncnt'] > 0:
                    ...
                    self.mft[i]['filename'] = mft.NO_FN_RECORD
